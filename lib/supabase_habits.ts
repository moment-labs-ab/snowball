...
export const insertHabit = async (
    ...
    if (name === "Habit" || frequency === 0) {
        return {
            success: false,
            message: "Please fill both name and frequency fields",
            data: undefined,
        };
    } else {
        ...
        if (error) {
            ...
            return {
                success: true,
                message: "Habit inserted successfully",
                data: data as Habit,
            };
        }
    }
...
export const updateHabitOrder = async (
    ...
    try {
        ...
        if (error) {
            ...
            return {
                success: false,
                message: "Error updating habit order",
                data: error,
            };
        }
        ...
        return {
            success: false,
            message: "Unexpected error updating habit order",
            data: error,
        };
    }
};
...
//Defining the types for handling habit changes.
type HabitTrackingChangeHandler = (payload: {
    eventType: string;
    new: HabitTracking;
    old: HabitTracking;
}) => void;
 ...
 * @returns
 */
export const listenToHabitTrackingTable = (
    handleChange: HabitTrackingChangeHandler
) => {
    const client = useSupabaseClient();
...
export const getTrackingCount = async (
    ...
    const totalTrackingCount =
        existingTracking?.reduce((acc, curr) => acc + curr.tracking_count, 0) || 0;
    //console.log(existingTracking)

    return totalTrackingCount;
};

export const getTrackingCountDates = async (
    habit_id: string,
    user_id: string,
    date: Date
) => {
...
export const getUserArchivedHabits = async (
    userId: string
): Promise<Habit[]> => {
    const client = useSupabaseClient();
