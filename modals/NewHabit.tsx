...
import { View, Text, TextInput, Button, Switch, Platform, TouchableOpacity, Alert, ScrollView, SafeAreaView } from 'react-native';
...
import { insertHabit } from '@/lib/supabase';
...
const NewHabit: React.FC<NewHabitProps> = ({ visible, onClose}) => {
  ...
  const submit = async () =>{
    ...
    try{
      const result = await insertHabit(user.userId, habit.name,
        habit.reminder, habit.frequency, habit.frequency_rate)
